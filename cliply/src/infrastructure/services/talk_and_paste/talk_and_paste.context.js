...
import {
  post_a_message_Request,
  posting_new_message_request,
} from "./talk_and_paste.requests";

import { GlobalContext } from "../global/global.context";
import { TextClipsContext } from "../home/text_clips.context";

export const TalkAndPasteContext = createContext();

export const TalkAndPasteContextProvider = ({ children }) => {
  ...
  const posting_new_text_clip_to_upload = async (new_text_clip) => {
    ...
    setIsLoading(true);
    try {
      const response = await posting_new_message_request(new_text_clip);
